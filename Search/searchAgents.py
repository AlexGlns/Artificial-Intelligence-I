...
class CornersProblem(search.SearchProblem):
    ...
    def __init__(self, startingGameState):
        ...
        "*** YOUR CODE HERE ***"
        self.start = (startingGameState.getPacmanPosition(),self.corners)  #start state gets Starting position and location of the four Corners
    ...
    def expand(self, state):
        ...
        for action in self.getActions(state):
            ...
            action_cost = self.getActionCost(state, action, next_state)  #get Action's cost
            children.append( ( next_state, action, action_cost) )   #Create list of triples
        
        self._expanded += 1 # DO NOT CHANGE
        return children         #Return a list of triples, (child, action, stepCost)
    ...
    def getNextState(self, state, action):
        ...
        if (nextx,nexty) in state[1]:       #check if next state is a corner
            temp=[]
            for (X,Y) in state[1]:
                if (X,Y)!=(nextx,nexty):
                    temp.append((X,Y))
            temp=tuple(temp)
            return ((nextx,nexty),temp)
...
def cornersHeuristic(state, problem):
    ...
    if (xstate,ystate) in state[1]:
        return 0

    for corner in state[1]:
        tempx=int(corner[0])
        tempy=int(corner[1])
        h.append(abs(xstate - tempx) + abs(ystate - tempy))         #Manhattan distance to unvisited corners
...
def foodHeuristic(state, problem):
    ...
    "*** YOUR CODE HERE ***"
    FoodCordinates=foodGrid.asList()

    if position in FoodCordinates:
        return 0

    Distances=[]
    for (x,y) in FoodCordinates:
        Distances.append(abs((int(position[0])-int(x))+(int(position[1])-int(y))))
...
class AnyFoodSearchProblem(PositionSearchProblem):
    ...
    def isGoalState(self, state):
        ...
        "*** YOUR CODE HERE ***"
        Cordinates=self.food.asList()
        Foods=[]
        flag=0
        for (xf,yf) in Cordinates:
            Foods.append(abs((x-xf)+(y-yf)))    #closest distance from current state to each dot
            if Foods[flag]==min(Foods):
                Goal=Cordinates[flag]       #save closest dot as Goal
            flag+=1
        if (x,y)==Goal:
            return True
        else:
